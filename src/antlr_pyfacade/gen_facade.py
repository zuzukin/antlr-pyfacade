...
def generate(parser_qualname: str, grammar: str) -> str:
    ...
    GrammarEventListener = f"{grammar.capitalize()}EventListener"
    ...
    return dedent(
        ...
        """
        Event-listener facade for the {grammar} grammar. Subclass {GrammarEventListener},
        ...
        class {GrammarEventListener}(FacadeListener):
            ruleNames = {rule_names!r}
            START_RULE = 0  # {rule_names[0]}
            ...
            def walk(
                ...
                *, 
                ...
                filtered: bool = True,
            ) -> {GrammarEventListener}:
                parser_spec, lexer_spec = load_specs(lexer_cls, parser_cls)
                rule = self._resolve_start_rule({grammar}EventListener, start_rule)
                drive(self, {GrammarEventListener}, parser_spec, lexer_spec, text, rule, filtered=filtered)
                return self
        '''
    ).lstrip()
