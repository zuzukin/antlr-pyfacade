...
def _incompatible_antlr(cls: type, e: Exception) -> Exception:
    ...
    fails here — the two choke points every parse path goes through. The raw
    errors (a C++ deserializer message, a bare AttributeError) name no cause or
    fix, so re-raise with one; the original is chained for the details.
    """
    ...
    if isinstance(e, AttributeError):
        return TypeError(
            ...
            f"parser/lexer (missing `{e.name}`) — it may have been generated by "
