...
def _serial(chunks, **kw):
    return [JsonValueBuilder().walk(c, start_rule="value").result for c in chunks]


def test_results_match_and_preserve_order():
    listeners = JsonValueBuilder.walk_parallel(_CHUNKS, start_rule="value")
...
def test_start_rule_name_and_index_agree():
    ...
    by_name = JsonValueBuilder.walk_parallel(_CHUNKS, start_rule="value")
    by_index = JsonValueBuilder.walk_parallel(_CHUNKS, start_rule=value_idx)
    assert [ln.result for ln in by_name] == [ln.result for ln in by_index]


def test_single_worker_inline_path_matches_pool():
    pooled = JsonValueBuilder.walk_parallel(_CHUNKS, start_rule="value", max_workers=4)
    inline = JsonValueBuilder.walk_parallel(_CHUNKS, start_rule="value", max_workers=1)
...
def test_factory_is_used_per_chunk():
    ...
    listeners = list(
        JsonValueBuilder.walk_parallel(_CHUNKS, start_rule="value", factory=factory)
    )
...
def test_unknown_start_rule_raises():
    with pytest.raises(ValueError, match="unknown start rule"):
        JsonValueBuilder.walk_parallel(_CHUNKS, start_rule="nonesuch")


def test_syntax_errors_collected_per_chunk():
    chunks = ['{"good": 1}', "{bad", '{"also_good": 2}']
    listeners = list(JsonValueBuilder.walk_parallel(chunks, start_rule="value"))
...
def test_source_positions_across_chunks():
    ...
    recs = list(_PosRecorder.walk_parallel(["[1,\n22]", "99"], start_rule="value"))
    ...
    with pytest.raises(TypeError, match="str or Chunk"):
        list(_PosRecorder.walk_parallel([123], start_rule="value"))
